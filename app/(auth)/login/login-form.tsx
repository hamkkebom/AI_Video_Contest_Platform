...
export default function LoginForm() {
  ...
  /* 인앱 브라우저 감지 (Google OAuth가 차단되는 환경) */
  const [isInAppBrowser, setIsInAppBrowser] = useState(false);
  useEffect(() => {
    if (typeof navigator === 'undefined') return;
    const ua = navigator.userAgent || '';
    // 카카오톡, 인스타그램, 페이스북, 네이버, 라인, 트위터 등 인앱 브라우저
    const inApp = /KAKAOTALK|Instagram|FBAN|FBAV|NAVER|Line|Twitter|Snapchat|Whale\/\d.*Puffin|DaumApps|GSA\/|WebView|wv\)/i.test(ua);
          ...
          <CardContent className="pt-4 space-y-4">
            ...
            {isInAppBrowser ? (
              <div className="rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800 p-3 space-y-1.5">
                <p className="text-sm font-semibold text-amber-800 dark:text-amber-200">Google 로그인은 기본 브라우저에서만 가능합니다</p>
                <p className="text-xs text-amber-600 dark:text-amber-400">
                  현재 인앱 브라우저에서 접속 중입니다. 우측 상단 메뉴(⋮)에서 &quot;기본 브라우저로 열기&quot;를 선택하거나, Safari/Chrome에서 직접 접속해주세요.
                </p>
              </div>
            ) : (
              <Button
