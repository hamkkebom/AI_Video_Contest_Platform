...
| 주최자 온보딩 (2026-08-01) | ⚠️ 마이그레이션 048·049 적용 대기 | `/hosts/apply` 신청 → `/admin/companies` 승인 → host 역할 자동 부여 (D-015) | RPC `apply_host_company`, `admin_set_company_status` |
...
| 비회원 문의 답변 메일 | ✅ 운영 | `lib/email/`, `app/api/admin/inquiries/[id]/` | `inquiries.guest_email` |
| 심사위원 초대 (미가입자) | ✅ 운영 | `app/api/judges/invite/`, `app/(public)/judge-invite/[token]/` | `judge_invites` (055) |
