-- ============================================================
-- 꿈플 AI 영상 공모전 플랫폼 — 통합 스키마 마이그레이션
-- 신 프로덕션 Supabase 프로젝트용 (전체 001~021 통합)
-- 생성일: 2026-03-03
-- ============================================================
...
CREATE TABLE profiles (
  ...
  preferred_chat_ai TEXT[] DEFAULT '{}',
  preferred_image_ai TEXT[] DEFAULT '{}',
  preferred_video_ai TEXT[] DEFAULT '{}',
...
CREATE TABLE contests (
  ...
  promotion_video_urls TEXT[] DEFAULT '{}',
  ...
  result_format TEXT DEFAULT 'website',
  guidelines TEXT,
  notes TEXT,
  hero_image_url TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
...
-- target_type: CHECK 제약 없음 (다양한 값 허용)
-- target_type, target_id: nullable (search 등 타겟 없는 액션 허용)
-- ============================================================
...
CREATE INDEX idx_utm_visits_user ON utm_visits(user_id);
CREATE INDEX idx_utm_visits_source ON utm_visits(utm_source);
CREATE INDEX idx_utm_visits_created ON utm_visits(created_at);

-- ============================================================
-- 25) account_withdrawals — 회원 탈퇴 사유
-- ============================================================
...
CREATE INDEX idx_withdrawals_user ON account_withdrawals(user_id);
...
-- 관리자 대리 등록
CREATE POLICY "submissions: 관리자 대리 생성" ON submissions FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.roles::text LIKE '%admin%')
);
-- 관리자/주최자 상태 변경
CREATE POLICY "submissions: 관리자/주최자 상태 변경" ON submissions FOR UPDATE USING (
  EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND (profiles.roles::text LIKE '%admin%' OR profiles.roles::text LIKE '%host%'))
);
...
-- 관리자 대리 생성
CREATE POLICY "bonus_entries: 관리자 대리 생성" ON bonus_entries FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.roles::text LIKE '%admin%')
);
...
-- INSERT 정책: 완화됨 (/api/log 라우트에서 인증 검증)
CREATE POLICY "activity_logs: 로그 삽입 허용" ON activity_logs FOR INSERT WITH CHECK (true);
...
CREATE POLICY "ip_logs: 로그 삽입 허용" ON ip_logs FOR INSERT WITH CHECK (auth.uid() = user_id);
...
-- ============================================================
-- 데이터 섹션 (021 기준)
-- auth.users + auth.identities + profiles + contests + 수상/가산점 구성
-- submissions/bonus_entries/utm_visits 데이터는 의도적으로 포함하지 않음
-- ============================================================
...
-- ============================================================
-- 7. 모든 시퀀스 리셋 (최종 데이터 기준)
-- ============================================================
SELECT setval(pg_get_serial_sequence('profiles', 'seq_id'), COALESCE((SELECT MAX(seq_id) FROM profiles), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('companies', 'id'), COALESCE((SELECT MAX(id) FROM companies), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('company_members', 'id'), COALESCE((SELECT MAX(id) FROM company_members), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('contests', 'id'), COALESCE((SELECT MAX(id) FROM contests), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('contest_award_tiers', 'id'), COALESCE((SELECT MAX(id) FROM contest_award_tiers), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('contest_bonus_configs', 'id'), COALESCE((SELECT MAX(id) FROM contest_bonus_configs), 0) + 1, false);
...
SELECT setval(pg_get_serial_sequence('judges', 'id'), COALESCE((SELECT MAX(id) FROM judges), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('judging_templates', 'id'), COALESCE((SELECT MAX(id) FROM judging_templates), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('judging_criteria', 'id'), COALESCE((SELECT MAX(id) FROM judging_criteria), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('scores', 'id'), COALESCE((SELECT MAX(id) FROM scores), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('score_criteria', 'id'), COALESCE((SELECT MAX(id) FROM score_criteria), 0) + 1, false);
...
SELECT setval(pg_get_serial_sequence('inquiries', 'id'), COALESCE((SELECT MAX(id) FROM inquiries), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('agency_requests', 'id'), COALESCE((SELECT MAX(id) FROM agency_requests), 0) + 1, false);
...
SELECT setval(pg_get_serial_sequence('ip_logs', 'id'), COALESCE((SELECT MAX(id) FROM ip_logs), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('account_withdrawals', 'id'), COALESCE((SELECT MAX(id) FROM account_withdrawals), 0) + 1, false);
